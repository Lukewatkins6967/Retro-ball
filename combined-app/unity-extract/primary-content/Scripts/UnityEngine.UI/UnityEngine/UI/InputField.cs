	...
	public class InputField : Selectable, IEventSystemHandler, IPointerClickHandler, IBeginDragHandler, IDragHandler, IEndDragHandler, IUpdateSelectedHandler, ISubmitHandler, ICanvasElement
	{
		...
		public Vector2 ScreenToLocal(Vector2 screen)
		{
			...
			else if (canvas.worldCamera != null)
			{
				...
				float enter;
				new Plane(m_TextComponent.transform.forward, m_TextComponent.transform.position).Raycast(ray, out enter);
		...
		public virtual void OnDrag(PointerEventData eventData)
		{
			...
			{
				Vector2 localPoint;
				RectTransformUtility.ScreenPointToLocalPointInRectangle(textComponent.rectTransform, eventData.position, eventData.pressEventCamera, out localPoint);
		...
		protected EditState KeyPressed(Event evt)
		{
			...
			bool flag = ((platform != RuntimePlatform.OSXEditor && platform != RuntimePlatform.OSXPlayer) ? ((modifiers & EventModifiers.Control) != 0) : ((modifiers & EventModifiers.Command) != 0));
