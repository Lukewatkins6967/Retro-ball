...
namespace UnityEngine.Networking
{
	public class NetworkServerSimple
	{
		...
		public void ListenRelay(string relayIp, int relayPort, NetworkID netGuid, SourceID sourceId, NodeID nodeId)
		{
			...
			Update();
			byte error;
			NetworkTransport.ConnectAsNetworkHost(m_ServerHostId, relayIp, relayPort, netGuid, sourceId, nodeId, out error);
			m_RelaySlotId = 0;
			if (LogFilter.logDebug)
			{
		...
		public void Update()
		{
			...
			do
			{
				...
				int channelId;
				int receivedSize;
				networkEventType = NetworkTransport.ReceiveFromHost(m_ServerHostId, out connectionId, out channelId, m_MsgBuffer, m_MsgBuffer.Length, out receivedSize, out error);
		...
		private void HandleConnect(int connectionId, byte error)
		{
			...
			NodeID dstNode;
			byte error2;
			NetworkTransport.GetConnectionInfo(m_ServerHostId, connectionId, out address, out port, out network, out dstNode, out error2);
