...
namespace UnityEngine.Networking
{
	public class ClientScene
	{
				...
				for (int i = 0; i < s_Peers.Length; i++)
				{
					...
					for (int j = 0; j < peerInfoMessage.playerIds.Length; j++)
					{
						...
						if (extraMessage != null)
						{
							...
							reconnectMessage.msgSize = networkWriter2.Position;
						}
		...
		private static void OnObjectSpawn(NetworkMessage netMsg)
		{
			...
			SpawnDelegate handler;
			NetworkIdentity uv;
			if (s_NetworkScene.GetNetworkIdentity(s_ObjectSpawnMessage.netId, out uv))
			{
		...
		private static void OnObjectSpawnScene(NetworkMessage netMsg)
		{
			...
			NetworkIdentity uv;
			if (s_NetworkScene.GetNetworkIdentity(s_ObjectSpawnSceneMessage.netId, out uv))
			{
		...
		private static void OnObjectDestroy(NetworkMessage netMsg)
		{
			...
			NetworkIdentity uv;
			if (s_NetworkScene.GetNetworkIdentity(s_ObjectDestroyMessage.netId, out uv))
			{
				uv.OnNetworkDestroy();
		...
		private static void OnUpdateVarsMessage(NetworkMessage netMsg)
		{
			...
			NetworkIdentity uv;
			if (s_NetworkScene.GetNetworkIdentity(networkInstanceId, out uv))
			{
		...
		private static void OnOwnerMessage(NetworkMessage netMsg)
		{
			...
			PlayerController playerController;
			if (netMsg.conn.GetPlayerController(s_OwnerMessage.playerControllerId, out playerController))
			{
				playerController.unetView.SetNotLocalPlayer();
			}
			NetworkIdentity uv;
			if (s_NetworkScene.GetNetworkIdentity(s_OwnerMessage.netId, out uv))
			{
