		...
		private void OnGUI()
		{
			...
			if (manager.matchInfo == null)
			{
				if (manager.matches == null)
				{
					...
					{
						manager.matchMaker.CreateMatch(manager.matchName, manager.matchSize, true, string.Empty, string.Empty, string.Empty, 0, 0, manager.OnMatchCreate);
					}
					num2 += 24;
					...
					{
						manager.matchMaker.ListMatches(0, 20, string.Empty, false, 0, 0, manager.OnMatchList);
					}
					num2 += 24;
				}
				else
				{
					for (int i = 0; i < manager.matches.Count; i++)
					{
						...
						if (GUI.Button(new Rect(num, num2, 200f, 20f), "Join Match:" + matchInfoSnapshot.name))
						{
							...
							manager.matchMaker.JoinMatch(matchInfoSnapshot.networkId, string.Empty, string.Empty, string.Empty, 0, 0, manager.OnMatchJoined);
						}
						num2 += 24;
					}
