...
namespace UnityEngine.Networking
{
	...
	public class NetworkDiscovery : MonoBehaviour
	{
		...
		public bool StartAsServer()
		{
			...
			byte error;
			if (!NetworkTransport.StartBroadcastDiscovery(m_HostId, m_BroadcastPort, m_BroadcastKey, m_BroadcastVersion, m_BroadcastSubVersion, m_MsgOutBuffer, m_MsgOutBuffer.Length, m_BroadcastInterval, out error))
			{
				if (LogFilter.logError)
				{
		...
		private void Update()
		{
			...
			do
			{
				...
				int receivedSize;
				byte error;
				networkEventType = NetworkTransport.ReceiveFromHost(m_HostId, out connectionId, out channelId, m_MsgInBuffer, 1024, out receivedSize, out error);
				if (networkEventType == NetworkEventType.BroadcastEvent)
				{
					...
					string address;
					int port;
					NetworkTransport.GetBroadcastConnectionInfo(m_HostId, out address, out port, out error);
