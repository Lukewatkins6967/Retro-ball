...
namespace UnityEngine
{
	public sealed class RectTransformUtility
	{
		...
		{
			Vector2 output;
			PixelAdjustPoint(point, elementTransform, canvas, out output);
			return output;
		}
		...
		{
			Rect value;
			INTERNAL_CALL_PixelAdjustRect(rectTransform, canvas, out value);
			return value;
		}
		...
		public static void FlipLayoutAxes(RectTransform rect, bool keepPositioning, bool recursive)
		{
			...
			if (recursive)
			{
				for (int i = 0; i < rect.childCount; i++)
				{
					...
					if (rectTransform != null)
					{
						FlipLayoutAxes(rectTransform, false, true);
					}
				}
			}
